...
export function RequestForm({
  ...
  const [descriptions, setDescriptions] = useState<DescriptionField[]>(() => [emptyField()]);
  const [previews, setPreviews] = useState<{ file: File; url: string | null }[]>([]);

  const [error, formAction, isPending] = useActionState(createSupportRequest, undefined);
      ...
      <DialogContent className="max-w-lg">
        ...
        >
          ...
          <div className="flex flex-col gap-2">
            <Label className="text-[13px] font-medium text-foreground">Descriptions (one per request)</Label>
          ...
          <div className="flex flex-col gap-1.5">
            <Label className="text-[13px] font-medium text-foreground">Files</Label>
            <input
              ref={fileInputRef}
              type="file"
              accept={FILE_ACCEPT}
